...
format:
  html:
    citeproc: true   # Enables citation rendering
...
The summary c-statistic is again estimated to be `r sprintf("%.2f", fit$est)`, with a 95% prediction interval ranging from `r sprintf("%.2f", fit$pi.lb)` to `r sprintf("%.2f", fit$pi.ub)`, reflecting expected variation in a new study. The posterior probability that the c-statistic in a future study exceeds 0.7 is `r sprintf("%.2f", fit$pr_cstat_gt_min)`.
