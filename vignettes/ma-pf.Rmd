...
ds <- Zhang %>% filter(outcome == "OS")

# Generate a forest plot of the log hazard ratio
metamisc::forest(theta = ds$HR, 
                 theta.ci.lb = ds$HR.025, 
                 theta.ci.ub = ds$HR.975, 
                 theta.slab = ds$Study, 
...
```{r,message=F,warning=F,echo=T,eval=T}
ds <- ds %>% mutate(logHR = log(HR), 
                    se.logHR =  (log(HR.975) - log(HR.025))/(2*qnorm((1 - 0.05/2))))
```
...
```{r,message=F,warning=F,echo=T,eval=T}
res <- rma(yi = logHR, sei = se.logHR, method = "FE", data = ds)
...
```{r,message=F,warning=F,echo=T,eval=T}
regtest(x = ds$logHR, sei = ds$se.logHR, model = "lm", predictor = "sei")
```

```{r,message=F,warning=F,echo=F,eval=T}
regfit = regtest(x = ds$logHR, 
                 sei = ds$se.logHR, 
                 model = "lm", 
                 predictor = "sei")
```
...
```{r}
regfit <- fat(b = ds$logHR, b.se = ds$se.logHR, method = "E-FIV")
regfit
```
...
```{r,message=F,warning=F}
regtest(x = ds$logHR, sei = ds$se.logHR, ni = ds$N, 
        model = "lm", 
        predictor = "ninv")
```
...
The pooled estimate for the log hazard ratio is `r sprintf("%.3f", resREML$b)` with a standard error of `r sprintf("%.3f", sqrt(vcov(resREML)))`. The between-study standard deviation of the log hazard ratio is `r sprintf("%.3f", sqrt(resREML$tau2))`. We can calculate the summary estimate for the hazard ratio and its corresponding 95% CI by back-transforming the estimate for $\mu$ and its confidence bounds:
...
The summary HR of HER2 is statistically significant, indicating that increased levels of HER2 are associated with poorer survival.
...
```{r, echo = F, message=F,warning=F,echo=T,eval=T}
probOS <- 1-pt((log(1.5) - mu)/sqrt(tau2+sigma2), df=(resREML$k-2))
```

The probability that the HR for HER2 will be >1.5 in a new setting is `r sprintf("%.0f%%", probOS*100)`. We can also estimate this probability by means of simulation:
...
We first derive the log hazard ratio and corresponding standard error:
...
```{r,message=F,warning=F,echo=T,eval=T}
resPFS <- rma(yi = Zhang$logHR, sei = Zhang$se.logHR, method = "REML", slab = Zhang$Study)
resPFS
...
```{r,message=F,warning=F,echo=T,eval=T}
res.MV <- rma.mv(yi = logHR, V, mods = ~ outcome - 1, 
