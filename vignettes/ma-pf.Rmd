...
kable(out, col.names = c("Study", "Country", "Disease", "N", "Prognostic effect"),
      align = "lccrc")
```
...
An advantage of the forest plot is that allows a visual inspection of the available evidence and facilitates the identification of potential between-study heterogeneity.
...
regfit = regtest(x = ds$logHR, sei = ds$se.logHR, model="lm", predictor="sei")
```
...
The new summary estimate is `r sprintf("%.3f", (res.MV$beta["outcomeOS",]))` versus `r sprintf("%.3f", resREML$b)` with an SE of ", sprintf("%.3f", sqrt(vcov(res.MV)["outcomeOS","outcomeOS"])), "(versus ", sprintf("%.3f", sqrt(vcov(resREML))), "). Hence, we have gained some precision. ")}`
